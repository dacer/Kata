    ...
    companion object {
        ...
        fun saveAsync(context: Context, string: String, alias: String = "") {
            Observable.fromCallable {
                val historyDb = im.dacer.kata.core.data.HistoryDbHelper(context).writableDatabase
        ...
        fun get(db: SQLiteDatabase, limit: Int): List<im.dacer.kata.core.model.History> {
            val result = arrayListOf<im.dacer.kata.core.model.History>()
            val queryStarred = im.dacer.kata.core.model.History.FACTORY.select_all_starred()
            db.rawQuery(queryStarred.statement, queryStarred.args).use { cursor ->
                while (cursor?.moveToNext() == true) {
                    result.add(im.dacer.kata.core.model.History.SELECT_ALL_MAPPER.map(cursor))
                }
            }
            val query = im.dacer.kata.core.model.History.FACTORY.select_unstarred_limit(limit.toLong())
